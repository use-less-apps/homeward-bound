...
platform=$sdk/platforms/$(/bin/ls $sdk/platforms | grep android-28)
test -z "$platform" && \
	echo "platform 28 is needed to build against: try $SDK_HOME/bin/sdkmanager 'platforms;android-28'" && \
	exit 1
...
signAndroidApplicationPackageWithDebugKey() {
	 ...
	 # jarsigner -keystore "$HOME/.android/useless.keystore" -storepass 'aoeu.htns' "$filepathOfUnalignedAPK" uselessprodkey 1>&2 2>/dev/null
}
